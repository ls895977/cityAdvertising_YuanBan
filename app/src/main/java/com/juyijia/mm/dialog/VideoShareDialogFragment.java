...
import com.juyijia.mm.activity.VideoReportActivity;
import com.juyijia.mm.adapter.VideoShareAdapter;
...
import com.juyijia.mm.mob.MobBean;
    ...
    public void onItemClick(MobBean bean, int position) {
        ...
        switch (bean.getType()) {
            case Constants.LINK://复制链接
                ((AbsVideoActivity) mContext).copyLink(mVideoBean);
                break;
            case Constants.REPORT://举报
                VideoReportActivity.forward(mContext, mVideoBean.getId());
                break;
            case Constants.SAVE://保存
                ((AbsVideoActivity) mContext).downloadVideo(mVideoBean);
                break;
            case Constants.DELETE://删除
                ((AbsVideoActivity) mContext).deleteVideo(mVideoBean);
                break;
            default:
                ((AbsVideoActivity) mContext).shareVideoPage(bean.getType(), mVideoBean);
                break;
        }
    }
